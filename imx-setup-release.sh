...
echo "UBOOT_CONFIG=\"sd epdc\"" >> conf/local.conf
echo "IMAGE_INSTALL:append = \" i2c-tools imx-test lua swupdate swupdate-progress swupdate-client swupdate-tools-ipc u-boot-imx u-boot-fw-utils systemd-swusys json-c coreutils util-linux\"" >> conf/local.conf
echo "IMAGE_FSTYPES = \" ext4 ext4.gz wic.bmap wic.gz\"" >> conf/local.conf

echo "CORE_IMAGE_EXTRA_INSTALL += \"maptattoo\"" >> conf/local.conf
...
echo "BBLAYERS += \"\${BSPDIR}/layers/meta-maptattoo\"" >> $BUILD_DIR/conf/bblayers.conf
echo "BBLAYERS += \"\${BSPDIR}/layers/meta-swupdate\"" >> $BUILD_DIR/conf/bblayers.conf
echo "BBLAYERS += \"\${BSPDIR}/layers/meta-swupdate-imx\"" >> $BUILD_DIR/conf/bblayers.conf
